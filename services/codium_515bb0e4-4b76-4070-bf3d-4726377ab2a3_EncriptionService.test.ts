...
describe('getHashedToken_method', () => {
  // Tests that hashing non-string input throws an error
  it('test_general_behaviour_non_string_input', async () => {
    const token = 123;

    await expect(Encryption.getHashedToken(token)).rejects.toThrow();
  });
});
